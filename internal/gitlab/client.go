...
func (c *Client) doRequest(ctx context.Context, method, endpoint string, body io.Reader, result interface{}) error {
	urlStr, err := c.endpointURL(endpoint)
	if err != nil {
		return err
	}
...
func (c *Client) endpointURL(endpoint string) (string, error) {
	...
	endpointPath := rel.Path
	out.Path = path.Join(basePath, endpointPath)
	out.RawPath = ""
	if c.baseURL.RawPath != "" || rel.RawPath != "" {
		baseRawPath := strings.TrimRight(c.baseURL.EscapedPath(), "/")
		out.RawPath = path.Join(baseRawPath, rel.EscapedPath())
	}
